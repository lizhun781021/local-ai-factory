...
### 📖 AI 工厂说明页内容更新（同步 v2.3.0 变更）

- 功能模块表：全部耗时任务标注"后台执行、切换菜单不丢"，Token 统计标注持久化，新增一键导出
- 模型清单：新增 Xing4.0（星辰语义大模型 8089）、Qwen-Image、星辰慧记 ASR
- 智能路由：新增"高难度任务 → Xing4.0"规则
- 服务架构：新增视觉识别(8081)、Xing4.0(8089)、智能路由(8606)、公网入口(8500+nginx限流)
- 数据安全：补充 .env 密钥管理、公网限流说明
- 项目目录：新增 factory_api.py / factory.sh / healthcheck.sh / tests / .env
- 顶部新增"后台任务特性"提示
