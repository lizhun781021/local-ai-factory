...
# 密钥一律从 .env 读取（.env 已被 .gitignore，不入库）
...
# ==================== Token 统计（持久化，重启不丢）====================
TOKEN_STATS_FILE = os.path.join(OUTPUT_DIR, "stats", "token_stats.json")

def _token_stats_save():
    """Token 统计落盘（每次更新后调用），webui 重启后自动恢复"""
    try:
        os.makedirs(os.path.dirname(TOKEN_STATS_FILE), exist_ok=True)
        with open(TOKEN_STATS_FILE, "w", encoding="utf-8") as f:
            json.dump({
                "total_tokens": st.session_state.token_stats["total_tokens"],
                "total_requests": st.session_state.token_stats["total_requests"],
                "start_time": st.session_state.token_stats["start_time"].strftime("%Y-%m-%d %H:%M:%S"),
                "history": list(st.session_state.token_stats["history"]),
            }, f, ensure_ascii=False, indent=1)
    except Exception:
        pass

if "token_stats" not in st.session_state:
    _ts = {"total_tokens": 0, "total_requests": 0, "history": deque(maxlen=100), "start_time": datetime.now()}
    try:
        if os.path.exists(TOKEN_STATS_FILE):
            with open(TOKEN_STATS_FILE, "r", encoding="utf-8") as f:
                _d = json.load(f)
            _ts["total_tokens"] = _d.get("total_tokens", 0)
            _ts["total_requests"] = _d.get("total_requests", 0)
            if _d.get("start_time"):
                try:
                    _ts["start_time"] = datetime.strptime(_d["start_time"], "%Y-%m-%d %H:%M:%S")
                except Exception:
                    pass
            _ts["history"] = deque(_d.get("history", [])[-100:], maxlen=100)
    except Exception:
        pass
    st.session_state.token_stats = _ts
    _token_stats_save()
...
def get_last_sync_info():
    ...
    log_path = os.path.expanduser("~/Desktop/星小辰工作空间/local-ai-factory/output/logs/rag_sync.log")
    try:
...
# 登录密码：仅从 .env 读取，代码不落默认值（.env 未配置时登录页会提示）
AUTH_PASSWORD = os.environ.get("AI_FACTORY_PASSWORD", "")
...
if not st.session_state.get("authed", False):
    ...
    with st.form("auth_form"):
        if not AUTH_PASSWORD:
            st.error("未配置访问密码：请在项目根目录 .env 中设置 AI_FACTORY_PASSWORD=你的密码")
            st.form_submit_button("登 录", use_container_width=True, disabled=True)
        else:
            _pw = st.text_input("访问密码", type="password", placeholder="请输入访问密码")
            _submitted = st.form_submit_button("登 录", use_container_width=True)
    if AUTH_PASSWORD and _submitted:
...
if page == "📊 系统监控":
    ...
    def _monitor_dashboard():
        ...
        try:
            ...
            _rc_path = os.path.join(PROJECT_DIR, "router_config.yaml")
            with open(_rc_path, "r", encoding="utf-8") as _f:
                _rc = _yaml.safe_load(_f)
            _models = {k: v for k, v in _rc.get("models", {}).items() if v.get("enabled", True)}
            if _models:
                _cols_n = min(len(_models), 4)
                _cols = st.columns(_cols_n)
                for _i, (_mname, _m) in enumerate(_models.items()):
                    with _cols[_i % _cols_n]:
                        _bu = _m.get("base_url", "")
                        _online = False
                        try:
                            _host = _bu.split("//")[1].split("/")[0] if "//" in _bu else _bu
                            _hp = _host.split(":")[0]
                            _pp = _host.split(":")[1] if ":" in _host else "80"
                            with _sock.create_connection((_hp, int(_pp)), timeout=1):
                                _online = True
                        except Exception:
                            _online = False
                        ...
                        if _online:
                            ...
                            st.markdown("<span style='color:#9ca3af;font-size:0.8rem'>○ 未启动</span>", unsafe_allow_html=True)
            else:
                st.info("路由配置中暂无启用模型")
        except Exception:
            st.info("无法读取路由配置")
...
# ==================== Token 统计页 ====================
    ...
    # 一键导出数据（日志/统计/报告打包下载）
    with st.expander("📦 一键导出数据（打包下载）"):
        st.caption("将操作日志、Token 统计、路由统计、测试报告等打包为 zip 下载")
        if st.button("📥 打包导出全部数据"):
            import zipfile
            _zip_path = os.path.join(OUTPUT_DIR, f"ai_factory_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip")
            _collected = 0
            with zipfile.ZipFile(_zip_path, "w", zipfile.ZIP_DEFLATED) as _z:
                # Token 统计 JSON
                if os.path.exists(TOKEN_STATS_FILE):
                    _z.write(TOKEN_STATS_FILE, "token_stats.json")
                    _collected += 1
                # 操作日志
                if os.path.exists(ACTIVITY_LOG_FILE):
                    _z.write(ACTIVITY_LOG_FILE, "activity.log")
                    _collected += 1
                # 路由统计
                _rs = os.path.join(OUTPUT_DIR, "logs", "router_stats.json")
                if os.path.exists(_rs):
                    _z.write(_rs, "router_stats.json")
                    _collected += 1
                # 同步日志
                _rl = os.path.join(OUTPUT_DIR, "logs", "rag_sync.log")
                if os.path.exists(_rl):
                    _z.write(_rl, "rag_sync.log")
                    _collected += 1
                # 测试报告目录
                if os.path.isdir(OUTPUT_REPORTS):
                    for _f in os.listdir(OUTPUT_REPORTS):
                        _fp = os.path.join(OUTPUT_REPORTS, _f)
                        if os.path.isfile(_fp):
                            _z.write(_fp, f"测试报告/{_f}")
                            _collected += 1
            st.success(f"✅ 打包完成：{_collected} 个文件")
            st.info(f"📁 已保存到：`{_zip_path}`")
            with open(_zip_path, "rb") as f:
                st.download_button("📥 另存为...", f, file_name=os.path.basename(_zip_path), mime="application/zip")
            log_activity("数据导出", f"打包 {_collected} 个文件")
        else:
            st.caption("包含：Token 统计 / 操作日志 / 路由统计 / 知识库同步日志 / 测试报告")
